...
import { getUserLocations } from "@/db/crud/locationCrud"
...
export const metadata = async () => {
  const translation = await getTranslTitle("userLocations")
...
const locationsUser = async () => {
  ...
  const locations = await getUserLocations(session?.user?.userId)
    ...
    <div>
      ...
      <div className="mt-10 space-y-2 mx-4 md:px-28">
        <h1 className="text-2xl font-bold">
          {t("yourlocations")} {locations?.length} {t("locations")}
        </h1>
